...
export default function EditSupervisorCheckPage() {
  ...
  const fetchCheck = async (id: string) => {
    try {
      ...
      setFormData({
        order: check.order?._id || check.order || '',
        drawing: check.drawing?._id || check.drawing || '',
        ...
        checkType: check.checkType || 'In-Process',
        dimensions: check.dimensions && check.dimensions.length > 0 ? check.dimensions : [{ parameter: '', specification: '', actualValue: '', tolerance: '', status: 'Pending', remarks: '' }],
        visualInspection: check.visualInspection || { status: 'Pending', remarks: '' },
        materialCheck: check.materialCheck || { status: 'Pending', remarks: '' },
        overallStatus: check.overallStatus || 'Pending',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${API_URL}/supervisor-check/${params.id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      router.push(`/admin/supervisor-check/${params.id}`);
    } catch (error: any) {
      alert(error.response?.data?.error || 'Error updating supervisor check');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-6xl mx-auto py-6 sm:px-6 lg:px-8">
          <div className="px-4 py-6 sm:px-0">
            <h2 className="text-3xl font-bold text-gray-900 mb-6">Edit Supervisor Check</h2>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Drawing (Optional)</label>
                  <select
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="block text-sm font-medium text-gray-700">Dimension Checks</label>
                  <button
                    type="button"
              ...
              <div className="flex justify-end space-x-4">
                <Link
                  href={`/admin/supervisor-check/${params.id}`}
