...
export default function EditDCPage() {
  ...
  const fetchDC = async (id: string) => {
    try {
      ...
      setFormData({
        ...
        partyAddress: dc.partyAddress || '',
        partyContact: dc.partyContact || '',
        vehicleNumber: dc.vehicleNumber || '',
        driverName: dc.driverName || '',
        driverContact: dc.driverContact || '',
        remarks: dc.remarks || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/dc/${params.id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      router.push(`/admin/dc/${params.id}`);
    } catch (error: any) {
      alert(error.response?.data?.error || 'Error updating DC');
    } finally {
      setLoading(false);
    }
  };
